...
public class AddPlayListCommand extends ListenerAdapter {
    @Override
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        String[] mess = event.getMessage().getContentRaw().split(" ", 3);

        Cmd cmd = new Cmd("addplaylist", new String[] {"apl"});

        if (mess.length == 3 && cmd.checkMess(mess[0])) {
            try {
                String folderPath = "playlist/" + event.getGuild().getId();
                ...
                if (file.exists()) {
                    event.getChannel().sendMessage("Playlist " + mess[1] + " đã tồn tại :exclamation:").queue();
                    return;
                }
                ...
                if (!file.exists()) {
                    ...
                    if (!isUrl(mess[2])) {
                        ...
                        writer.write(mess[2]);
                        event.getChannel().sendMessage(":white_check_mark: Đã thêm playlist " + mess[1] + ": " + mess[2]).queue();
                        writer.close();
                    }
                }
        ...
        if (mess.length < 3 && cmd.checkMess(mess[0])) {
            ...
            emb.addField("Add Playlist", prefix + "addplaylist/" + prefix + "apl + tênPlaylist + linkNhạc", true);
